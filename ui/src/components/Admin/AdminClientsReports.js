...
const AdminClientReports = () => {
  ...
  const getData = async (orderStatus, fromDate, untilDate) => {
    await httpClient
      .get(
        `/orders/allOrders?statusId=${orderStatus}&fromDate=${fromDate}&untilDate=${untilDate}&clientId=${selectedClient}`
      )
      .then((res) => {
      ...
      <Box sx={boxStyle}>
        <Box>
          ...
          <Grid container spacing={1} sx={{ mt: 2 }} columns={30}>
            ...
            <Grid item xs={15}>
              <FormControl fullWidth>
                <InputLabel id="demo-simple-select-label">Klientët</InputLabel>
                <Select
                  labelId="demo-simple-select-label"
                  id="demo-simple-select"
